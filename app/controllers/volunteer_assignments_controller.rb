class VolunteerAssignmentsController < ApplicationController
  ...
  def index
    @assignments = current_user.volunteer_assignments.includes(:event).order(created_at: :desc)
  end

  def history
    @assignments = current_user.volunteer_assignments.completed.includes(:event).order(date_logged: :desc)
  ...
  def create
    ...
    if current_user.volunteer_assignments.where(event: event).where.not(status: %w[cancelled rejected]).exists?
    ...
    assignment = current_user.volunteer_assignments.build(event: event, status: "pending")
